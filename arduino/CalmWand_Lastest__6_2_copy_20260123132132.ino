...
// toolbox for controlling the colored lights on the wand //
#include <Adafruit_MLX90614.h>
// toolbox for reading the temperature sensor //
#include <ArduinoBLE.h>
// toolbox for bluetooth (BLE)//
#include <SD.h>
// toolbox for saving data to the SD card (stores session recordings)

// --- CONFIGURATION/settings section---
// #define menas whenever you see this word, replace it with this number //
#define Lower_Limit 7000 // coldest temperature wand cares about, 1st color below this //
#define Upper_Limit 9800 // hottest temp, above = last color //
#define PIN A1           // physical pin arduino connected to //
#define motor_pin A3     // vibration motor connected //
#define NUMPIXELS 7
#define MAX_LINE_LENGTH 80
#define BREATH_PIN A0
#define LOOP_INTERVAL 20 // code checked every 20 milliseconds //

Adafruit_NeoPixel pixels(NUMPIXELS, PIN, NEO_GRB + NEO_KHZ800);
Adafruit_MLX90614 mlx = Adafruit_MLX90614();

// --- BLE UUIDs ---
BLEService radarService("87f23fe2-4b42-11ed-bdc3-0242ac120000"); // main bluetooth channel, exact freq //
// BLERead/notify: recieve information //
// BLEWrite: send new settings to wand and change how it behaves //
// Characteristics each control one aspect - connection btwn arduino wand and app //
...
const float ColorArray[14][3] = {
    {0.25, 0.25, 0.25}, {0.3, 0, 0.3}, {0.95, 0, 0}, {0.95, 0.15, 0}, {0.95, 0.45, 0}, {1, 1, 0}, {0.35, 1, 0}, {0, 1, 0.02}, {0, 1, 0.2}, {0, 1, 1}, {0, 0.125, 1}, {0.15, 0, 1}, {0.6, 0, 1}, {1, 1, 1}};

float ColorArraySetup[6][3] = {
    {0.8, 0, 0}, {0.4, 0, 0.8}, {0, 0, 1}, {0, 0.5, 0.7}, {0, 1, 0}, {0, 0, 0}};

// --- VARIABLES ---
// math to figure out color system //
...
// float: floating point #, can be a decimal //
...
bool vibration = 0; // value either T/F //
...
// smooths out temperature readings, rolling avg. last 50 readings //
int GetOptimizedAverage(int newTemp)
{
  ...
  tempBufferIndex++;
  if (tempBufferIndex >= NumOfPoints)
    tempBufferIndex = 0;
  return tempRunningSum / NumOfPoints;
}
// what number to call next session file //
int getNextSessionId()
{
  ...
  // if shortcut file missing, manually counts all existing session files //
  int maxNum = -1;
  File root = SD.open("/");
  if (root)
  {
...
// void: func doesn't return anytying back, finishes //
...
// starts brand new recording session //
...
void setup()
{
  Serial.begin(115200); // opens comms btwn arduino + computer for debugging.
  // Wait for serial so we can see startup BLE logs, timeout after 2s if no USB
  while (!Serial && millis() < 2000)
    ;

  pinMode(motor_pin, OUTPUT); // output --> send signals out rather than in //
  ...
  for (int i = 0; i < NumOfPoints; i++)
    tempBuffer[i] = 0;
  // start up the SD card, begin a new session, and load saved settings //
  ...
  BLE.setAdvertisedService(radarService); // bluetooth broadcasting //
  // set the initial values on each characteristic
  ...
  // 1st loop (0-84) //
  // int i = 0: starts at 0 //
  // i < NumberOfColors * 6: keep going while its less than 84 //
  // i++: add 1 each time //
...
// figures out which two colors blend btwn based on current temp //
...
// figures out how many of the 7 lights should be on //
...
// sends saved session file from the SD card to the ap line by line over bluetooth //
void sendFileContent(const String &filename)
{
  Serial.print("BLE DEBUG: >> Sending file content: ");
  ...
  if (!cancelRequested)
  {
    Serial.print("BLE DEBUG: >> Transfer Complete. Bytes sent: ");
    Serial.println(byteCount);
    fileContentChar.setValue("EOF");
    delay(10);
  }
}
// checks if the app sent any new settings and applies them //
void handleBleCommands()
{
  ...
  if (brightnessCharacteristic.written())
  {
    ...
    Serial.print("BLE DEBUG: Write Brightness: ");
    Serial.println(val);
  ...
  if (inBreathTimeCharacteristic.written())
  {
    ...
    Serial.print("BLE DEBUG: Write InBreath: ");
  ...
  if (outBreathTimeCharacteristic.written())
  {
    ...
    BreathCycleTime = InbreathTime + OutbreathTime;
    Serial.print("BLE DEBUG: Write OutBreath: ");
  ...
  if (motorStrengthCharacteristic.written())
  {
    ...
    Serial.print("BLE DEBUG: Write Motor: ");
      ...
      if (root)
      {
        ...
        while (entry)
        {
          if (!entry.isDirectory())
          {
            ...
            Serial.print("BLE DEBUG: Listing File -> ");
            Serial.println(out);
  ...
  if (fileContentRequestChar.written())
  {
    ...
    Serial.print("BLE DEBUG: Content Req: ");
    Serial.println(cmd);
    ...
    else if (cmd.startsWith("DELETE:"))
    {
      ...
      if (SD.exists(filename))
      {
        ...
        if (SD.exists("nextsid.txt"))
          SD.remove("nextsid.txt");
        Serial.print("BLE DEBUG: File Deleted: ");
        Serial.println(filename);
      }
      else
      {
        Serial.print("BLE DEBUG: Delete failed, file missing: ");
        Serial.println(filename);
      }
    }
  }
  ...
  if (fileActionChar.written())
  {
    ...
    Serial.print("BLE DEBUG: Action Req: ");
    Serial.println(cmd);
    ...
    else if (cmd.startsWith("DELETE:"))
    {
      ...
      if (SD.exists(fname))
      {
        ...
        Serial.print("BLE DEBUG: Action Delete success: ");
        Serial.println(fname);
      }
      else
      {
        fileNameChar.setValue("ERROR:NOTFOUND:" + fname);
        Serial.print("BLE DEBUG: Action Delete fail (not found): ");
        Serial.println(fname);
      }
    }
  }
...
void loop()
{
  ...
  if (currentMillis - lastLoopTime >= LOOP_INTERVAL)
  {
    ...
    if ((NumberOfLedsOn > previous_num_led_on || (NumberOfLedsOn == 0 && previous_num_led_on == 5)) && vibration_progress[currentLedIndex] == 0)
    {
      vibration = 1;
      vibration_start = currentMillis;
      for (int i = 0; i <= currentLedIndex; i++)
      {
        vibration_progress[i] = 1;
      }
    }
    ...
    for (int i = 1; i <= NumberOfLedsOn; i++)
    {
